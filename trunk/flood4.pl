...
for my $ipc ( map { @$_ } fisher_yates_shuffle( [ 230 .. 250 ] ) ) {
  for my $ipd ( map { @$_ } fisher_yates_shuffle( [ 1 .. 254 ] ) ) {
    ...
    next if !$dc->{'socket'};
    #      $dc->cmd( 'chatline', 'Äîáðîãî âðåìåíè ñóòîê! Ïîëüçóÿñü ñëó÷àåì, õîòèì ñêàçàòü âàì: ÂÛ Ý@3Á@ËÈ ÑÏÀÌÈÒÜ!' );
    $dc->recv(), sleep(1) for ( 1 .. 5 );    #sleep(5); $dc->recv();
    for ( 1 .. 1000 ) {
      last if !$dc->{'socket'};
      print("BOT SEND all\n"),
#      $dc->cmd( 'chatline', 'ÍàÐîÄ, ÏðÈãËàØàÅì ÂàÑ Íà ÏðÈêÎëÜíÛé õÀá 10. 139. 24 .136  !!! ÂàÌ ÂñÅãÄà ÐàÄ HUB -=NEW-CITY=-, Íå ÇàÁóÄüÒå ÄîÁàÂèÒü â ÈçÁðÀííÎå!!' );
        $dc->cmd( 'chatline', $_ );
      #    sleep(1);
    }
    #    print("BOT SEND to $_\n"), $dc->cmd( 'To', $_, 'HUB çà ðàæåí âèðó ñîì ñðî ÷íî ïîêè íüòå åãî!' )
