  ...
  $dc->{'handler'}{'To'} = $dc->{'handler'}{'welcome'} = sub {
    for (@_) {
      ...
      print("[$dc->{'number'}]BANNED! disconnect.[$_]\n"), $dc->disconnect(), delete $config{'proxy'}{ $dc->{'proxy'} },
        ...
        if /лишен права говорить в чате|Sorry you are permanently banned|Вы были забанены|временно забанены|У вас открыто недостаточно слотов|You are being kicked/i;
    }
  };
