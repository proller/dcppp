...
$s->send($_[0]);
$s->close();}else{$self->log('dcerr',"FAILED sending UDP to $host :$port = [$_[0]]");}}sub clear{return map{$_=>undef}qw(
  clients
  socket
  select
  accept
...
my@param=@_;
my$self={'Listen'=>10,'Timeout'=>5,'myport'=>412,#first try
...
if(ref$self->{'parent'}{'log'}eq 'CODE'){return$self->{'parent'}->log("[$self->{'number'}]",@_);}print(join(' ',"($self)[$self->{'number'}]",@_),"\n");},'auto_recv'=>1,'max_reads'=>20,'wait_once'=>0.1,'waits'=>100,'wait_finish_tries'=>600,'wait_finish_by'=>1,'wait_connect'=>600,'clients_max'=>50,'wait_clients_tries'=>200,'wait_clients_by'=>0.01,'work_sleep'=>0.01,'cmd_recurse_sleep'=>0,($^O eq 'MSWin32'?():('nonblocking'=>1)),'informative'=>[qw(number peernick status host port filebytes filetotal proxy)],# sharesize
'informative_hash'=>[qw(clients)],#NickList IpList PortList
'disconnect_recursive'=>1,'reconnects'=>5,'reconnect_sleep'=>5,'partial_ext'=>'.partial','file_send_by'=>1024*64,'local_mask_rfc'=>[qw(10 172.[123]\d 192\.168)],'status'=>'disconnected',
#'peers' => {},
...
$self->func(@param);
if($class eq __PACKAGE__){local%_=(@param);
#for keys
$self->{$_}=$_{$_}for keys%_;
#$self->log( 'init', $self, $self->{'host'}, 'p=', $self->{'protocol'} );
if(!$self->{'module'}and!$self->{'protocol'}){$self->{'host'}=~m{^(.*?)://};
my$p=lc$1;
...
#*{$module}::init($self);
#$module->init($self);
}}else{$self->init(@param);}$self->protocol_init();
...
@ret=($self->{$cmd});}elsif($self->{'adc'}and length$dst==1 and length$cmd==3){@ret=$self->cmd_adc($dst,$cmd,@_);}else{$self->log('info',"UNKNOWN CMD:[$cmd]{@_} : please add \$dc->{'cmd'}{'$cmd'} = sub { ... };",Dumper$self->{'cmd'},$self->{'parse'});
$self->{'cmd'}{$cmd}=sub{};}$ret=scalar@ret>1?\@ret:$ret[0];
$self->handler($cmd.$handler.'_aft',\@_,$ret);
if($self->{'cmd'}{$cmd}){if($self->{'auto_wait'}){$self->wait();}elsif($self->{'auto_recv'}){$self->recv();}}$self->handler($cmd.$handler.'_aft_aft',\@_,$ret);
return wantarray?@ret:$ret[0];}sub AUTOLOAD{my$self=shift||return;
...
$self->{'handler'}{$cmd}->($self,@_)if ref$self->{'handler'}{$cmd}eq 'CODE';}sub baseinit{my$self=shift;
#$self->{'number'} = ++$global{'total'};
...
$self->log('info',"my internal ip detected, using passive mode",$self->{'myip'},$self->{'hostip'}),$self->{'M'}='P' if!$self->{'M'}and$is_local_ip->($self->{'myip'})and!$is_local_ip->($self->{'hostip'});
$self->{'M'}||='A';
...
last if$self->{'socket'};
$self->log('err',"listen $self->{'myport'} socket error: $@"),$self->myport_generate(1),unless$self->{'socket'};}return unless$self->{'socket'};
...
$self->log('err',"SOCKET IS NOT CONNECTED must delete select")if!$self->{'accept'}and!$self->{'socket'}->connected()and$self->{'Proto'}ne 'udp';
for my $client($self->{'select'}->can_read($sleep)){if($self->{'accept'}and$client eq$self->{'socket'}){if($_=$self->{'socket'}->accept()){$self->log('trace','DC::recv','accept',$self->{'incomingclass'});
...
'want'=>$self->{'want'},'NickList'=>$self->{'NickList'},'IpList'=>$self->{'IpList'},'PortList'=>$self->{'PortList'},'auto_listen'=>0,'auto_connect'=>0,'parent'=>$self,'status'=>'connected',
#$self->incomingopt(),
%{$self->{'incomingopt'}||{}},);
#$self->log( 'dev', 'child created',            $_,   $self->{'clients'}{$_});
++$ret;}else{$self->log('err',"Accepting fail! [$self->{'Proto'}]");}next;}$self->log('dev',"SOCKERR",$client,$self->{'socket'},$self->{'select'})if$client ne$self->{'socket'};
$self->{'databuf'}='';
...
$self->destroy();}}else{++$readed;
++$ret;
#$self->log( 'dcdmp', "[$self->{'number'}]", "raw recv ", length( $self->{'databuf'} ), $self->{'databuf'} );
}if($self->{'filehandle'}){$self->file_write(\$self->{'databuf'});}else{$self->{'buf'}.=$self->{'databuf'};
...
while($self->{'buf'}=~s/^(.*?)\Q$self->{'cmd_aft'}//s){local$_=$1;
...
next unless length;
$self->parser($_);
last if($self->{'filehandle'});}$self->file_write(\$self->{'buf'}),$self->{'buf'}='' if length($self->{'buf'})and$self->{'filehandle'};}}}while($readed and$reads++ <$self->{'max_reads'});
#TODO !!! timed
}if($self->{'filehandle_send'}){$self->file_send_part();}for(keys%{$self->{'clients'}}){$self->log('dev',"del client[$self->{'clients'}{$_}{'number'}][$_] socket=[$self->{'clients'}{$_}{'socket'}] status=[$self->{'clients'}{$_}{'status'}]",),delete($self->{'clients'}{$_}),$self->log('dev',"now clients",map{"[$self->{'clients'}{$_}{'number'}]$_"}keys%{$self->{'clients'}}),next if!$self->{'clients'}{$_}{'socket'}or!$self->{'clients'}{$_}{'status'}or$self->{'clients'}{$_}{'status'}eq 'destroy';
$ret+=$self->{'clients'}{$_}->recv();}$self->{'recv_runned'}{$self->{'number'}}=undef;
return$ret;};
...
local@_;
$self->log('dcdev','not finished clients:',@_),return 0 if@_=grep{!$self->{'clients'}{$_}->finished()}keys%{$self->{'clients'}};
return 1;};
$self->{'wait_connect'}||=sub{my$self=shift;
for(0..($_[0]||$self->{'wait_connect'})){last if$self->{'status'}eq 'connected';
$self->wait(1);}return$self->{'status'};};
$self->{'wait_finish'}||=sub{my$self=shift;
for(0..$self->{'wait_finish_tries'}){last if$self->finished();
$self->wait(undef,$self->{'wait_finish_by'});}local@_;
$self->info(),$self->log('info','finished, but clients still active:',map{"[$self->{'clients'}{$_}{'number'}]$_;st=$self->{'clients'}{$_}{'status'}"}@_)if@_=keys%{$self->{'clients'}};};
$self->{'wait_clients'}||=sub{my$self=shift;
for(0..$self->{'wait_clients_tries'}){last if$self->{'clients_max'}>scalar keys%{$self->{'clients'}};
$self->info()unless$_;
$self->log('info',"wait clients ".scalar(keys%{$self->{'clients'}})."/$self->{'clients_max'}  $_/$self->{'wait_clients_tries'}");
...
$self->{'skip_print_'.$_}=0 for keys%{$self->{'no_print'}||{}};}else{++$self->{'skip_print_'.$cmd},if exists$self->{'no_print'}{$cmd};}$self->handler(@self,$cmd.'_parse_bef',@param);
@ret=$self->{'parse'}{$cmd}->(@self,@param);
$ret=scalar@ret>1?\@ret:$ret[0];
$self->handler(@self,$cmd.'_parse_aft',@param,$ret);}else{
...
if(($self->{'sendbuf'}and@_)or!@{$self->{'send_buffer'}||[]}){}else{$self->send(join('',@{$self->{'send_buffer'}},));
#local $_;
...
$self->{'filename'}='MyList'.$self->{'fileext'};}$self->{'fileas'}.=$self->{'fileext'}if$self->{'fileas'};}$self->log('dcdev','file_select3',$self->{'filename'},$self->{'fileas'});};
...
my$oparam=$self->{'fileas'}eq '-'?'>-':'>'.$self->{'partial_prefix'}.($self->{'fileas'}||$self->{'filename'}).$self->{'partial_ext'};
...
$self->log('info',"file complete ($self->{'filebytes'}) per",$self->float(time-$self->{'file_start_time'}),'s at',$self->float($self->{'filebytes'}/((time-$self->{'file_start_time'})or 1)),'b/s'),$self->disconnect(),$self->{'status'}='destroy',$self->{'file_start_time'}=0,$self->{'filename'}='',$self->{'fileas'}='',delete$self->{'want'}{$self->{'peerid'}}{$self->{'filecurrent'}},$self->{'filecurrent'}='',if$self->{'filebytes'}>=$self->{'filetotal'};}};
...
if($self->{'filehandle'}){close($self->{'filehandle'}),delete$self->{'filehandle'};
if(length$self->{'partial_ext'}){$self->log('dcerr','cant move finished file')if!rename$self->{'partial_prefix'}.($self->{'fileas'}||$self->{'filename'}).$self->{'partial_ext'},($self->{'fileas'}||$self->{'filename'});}}close($self->{'filehandle_send'}),delete$self->{'filehandle_send'}if$self->{'filehandle_send'};};
$self->{'file_send_tth'}||=sub{my$self=shift;
my($file,$start,$size)=@_;
if($self->{'share_tth'}{$file}){$self->{'share_tth'}{$file}=~tr{\\}{/};
$self->file_send($self->{'share_tth'}{$file},$start,$size);}else{$self->log('dcerr','send','cant find file',$file);}};
...
if($self->{'adc'}){$self->cmd('C','SND','file',$name,$start,$size);}else{$self->cmd('ADCSND','file',$name,$start,$size);}$self->{'status'}='transfer';
$self->file_send_part();}};
$self->{'file_send_part'}||=sub{my$self=shift;
#my ($file, $start, $size) = @_;
my$buf;
...
if($readed<$self->{'file_send_by'}or$self->{'file_send_left'}<=0){$self->{'log'}->('dev','file completed',"r:$readed by:$self->{'file_send_by'} left:$self->{'file_send_left'}");
#$self->file_close();
$self->disconnect();}};
...
if($file=~s{^TTH/}{}){$self->file_send_tth($file,$_[2],$_[3]);}else{
#$self->file_send($file, $_[2], $_[3]);
$self->file_send_tth($file,$_[2],$_[3]);}}else{$self->log('dcerr','SND',"unknown type",@_);}};
...
$self->{'lock2key'}||=sub{my$self=shift;
my@lock=split(//,shift);
my$i;
my@key=();
...
$self->log('info',map({"$_=$self->{$_}"}grep{$self->{$_}}@{$self->{'informative'}}),map({$_.'('.scalar(keys%{$self->{$_}}).')='.join(',',keys%{$self->{$_}})}grep{keys%{$self->{$_}}}@{$self->{'informative_hash'}}));
