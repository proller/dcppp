#!/usr/bin/perl
#$Id: psmisc.pm 4854 2015-02-27 16:56:22Z pro $ $URL: svn://svn.setun.net/search/trunk/lib/psmisc.pm $
...
use lib::abs;
our $VERSION = ( split( ' ', '$Revision: 4854 $' ) )[1];
our (%config);
#my ( %config );
#local *config = *main::config;
#local
...
sub use_try ($;@) {
  ...
  return eval {
    return $INC{$path} or eval 'use ' . $module . ' qw(' . ( join ' ', @_ ) . ');1;' and $INC{$path};
  };
}
...
sub schedule($$;@) {    #$Id: psmisc.pm 4854 2015-02-27 16:56:22Z pro $ $URL: svn://svn.setun.net/search/trunk/lib/psmisc.pm $
  our %schedule;
